...
const Modify_post = () => {
    ...
    const getPostData = async () => {
        publishRefreshToken();
        const { data } = await axios.get('http://localhost:8000/api/post/mypostlist/', {
                ...
                <div className={styles.post_container}>
                    ...
                    <div className={styles.post_list}>
                        {postList.message !== '아직 작성한 글이 없습니다.' ? (
                            ...
                            <span className={styles.message__span}>{message}</span>
                        )}
                    </div>
                </div>
            </div>
